...
int loadModel(char *filename, ObjFile *file)
{
	...
	buf = new char[buf_size]; //TODO обработать исключение
		...
		if (strsize > 0)
		{
			...
			while (1)
			{
				...
				if (str.find("f ") == 0) //описание граней
				{
					...
					int pos = 1;
					deque<unsigned int> Vertexes;
					deque<unsigned int> TexCoords;
					deque<unsigned int> Normals;

					unsigned int v = 0, t = 0, n = 0;
					
					file->Faces.push_back(ObjFace());
					deque<ObjFace>::reverse_iterator it = file->Faces.rbegin();
